...

{
  ...
  # Включаем сервис wl-clip-persist
  services.wl-clip-persist = {
    enable = true;
    # Персистентность для основного буфера обмена.
    # Обратите внимание: работа с primary clipboard (средняя кнопка) может
    # иметь побочные эффекты для некоторых приложений[reference:6].
    clipboardType = "both";  # или "regular", если хотите только Ctrl+C/V
  };
