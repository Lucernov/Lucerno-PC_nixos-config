    ...
    # ./nixos/nx_steam.nix
{
  systemPackages = with pkgs; [
    ...
    sensors                                                     # Утилита для отображения температуры и состояния датчиков оборудования
    ...
    nmtui                                                       # Текстовый интерфейс для управления NetworkManager (настройка сетей)
    nload                                                       # Мониторинг сетевого трафика в реальном времени с графиками
    bandwich                                                    # Утилита для проверки скорости интернета из командной строки
