    ...
    # ./nixos/nx_steam.nix
{
  systemPackages = with pkgs; [
    ...
    findnixstorelinks                                           # Ищет симлинки в /nix/store и показывает, куда они ведут
    ...
    nix-index                                                   # Содержит nix-locate – поиск файлов в Nix store
    cachix                                                      # Утилита для работы с кэшем Nix (добавление/управление бинарными кэшами)
    nixos-repl                                                  # REPL (интерактивная оболочка) для NixOS
    any-nix-shell                                               # Позволяет использовать оболочки Nix из любого терминала
